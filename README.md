...
AgentInterposer keeps explicit built-in compatibility assertions for model/client combinations that have reproducible certification evidence. The `nvidia/nemotron-3-super-120b-a12b` profile asserts Chat Completions, native Responses, and tool calling, and records the hosted Codex CLI `0.147.0` and Claude Code CLI `2.1.226` certification scenarios described below. Vision input is intentionally not asserted for that model. The `meta/llama-3.2-11b-vision-instruct` profile asserts Chat Completions and vision input only, backed by a hosted randomized base64-image read through the Messages adapter; Responses and tool calling remain uncertified for that profile. The `openai/gpt-oss-120b` profile asserts Chat Completions and native Responses only, backed by repeated hosted `full` probes; tool calling, vision input, and external client/version scenarios remain uncertified for that profile.
...
4. Expand the model capability registry beyond the current Nemotron 3 Super, Llama 3.2 Vision, and GPT-OSS 120B evidence sets.
