...
> **Status:** stable v1. The compatibility boundary below is the supported v1 contract; compatibility breadth outside that boundary remains evidence-driven and may expand in later releases. The current foundation provides hardened OpenAI-compatible Chat Completions, native Responses passthrough, and an Anthropic Messages adapter for text, top-level and text-only mid-conversation system instructions, base64 and URL user image inputs, and custom client tools in both non-streaming and SSE streaming modes. Manual compatibility probes verify Codex CLI `0.147.0` over Responses for a single shell-tool round trip plus dependent two-tool and three-tool loops, and Codex CLI `0.148.0` for those scenarios plus a dependent four-tool loop, and Claude Code CLI `2.1.226` and `2.1.233` over Messages for single Bash-tool, dependent two-tool, and error-recovery flows, plus Claude Code CLI `2.1.235` for those established bare-mode scenarios, a dependent three-tool loop, and separately repeated normal-mode single-tool and dependent two-tool Bash round trips that preserve Claude Code's mid-conversation system turns, with `nvidia/nemotron-3-super-120b-a12b` through AgentInterposer. OpenCode CLI `1.18.18` is additionally certified over streaming Chat Completions for a dependent two-tool `read` -> `bash` proof with `nvidia/nemotron-3-super-120b-a12b`. A separate randomized hosted image probe certifies base64 Messages vision input with `meta/llama-3.2-11b-vision-instruct`. These are narrow certification profiles, not claims of universal agent or model compatibility.
...
- manual Claude Code CLI `2.1.226` and `2.1.233` certification for the established bare-mode Messages single Bash-tool, dependent two-tool, and error-recovery round trips, plus Claude Code CLI `2.1.235` certification for those scenarios, a dependent three-tool loop, and separately repeated normal-mode single-tool and dependent two-tool Bash round trips with mid-conversation system turns, with `nvidia/nemotron-3-super-120b-a12b`
...
3. Broaden Claude Code/Messages certification beyond the current bare-mode single-tool/dependent two-tool/dependent three-tool/error-recovery profiles and the normal-mode single-tool/dependent two-tool profiles to additional normal-mode loops, client versions, models, parallel tools, and longer multi-turn patterns.
